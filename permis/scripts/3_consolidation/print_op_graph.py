...
DEFAULT_GPKL = Path(__file__).resolve().parents[3] / "permis" / "data" / "0005804239" / "graphs" / "op_graph.gpickle"

def _load_graph(p: Path):
    with open(p, "rb") as fh:
        return pickle.load(fh)

def _year_from_doc(doc: Optional[str], node_name: Optional[str] = None, attrs: Optional[dict] = None) -> Optional[str]:
    """
...
def main():
    ...
    p.add_argument("--top", type=int, default=10, help="nombre de composantes à afficher (par taille)")

...
# TODO réécrire le prompt???? (ajout 1 entrée par modif)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Envoyer blocs au LLM et sauvegarder opérations brutes par AP"
    )
    p.add_argument("--no-api", action="store_true", help="ne pas appeler l'API (mode test)")
    p.add_argument(
        "--model", "-m", help="nom du modèle logique (ex: GPT5, GPT5mini, ou vide pour default)"
    )

...
CATALOGUE_PATH = PROJECT_PERMIS / "data" /  "0005804239" / "journaux" / "catalogue_ap.json" 
...
def split_blocs(filepath: Union[str, Path], target_per_block: int = 70000, max_blocks: int = 5) -> List[Dict]:
    ...
    - si un candidat est bien plus grand que target_per_block : tenter d'éclater par sous-sections / sections multiples
        ...
        if len(cand) > target_per_block * 2:
            ...
            if len(subs) > 1:
                ...
                if len(parts) > 1:
                    ...
                    continue
            # sinon, si ce fragment est une section contenant des sous-sections directes, prendre ces sous-sections
            top = soup_c.find("section")
            if top:
                child_secs = [s for s in top.find_all("section", recursive=False)]
                if child_secs:
                    parts = [normalize_html_minify_fragment(str(s)) for s in child_secs if str(s).strip()]
                    if len(parts) > 1:
                        expanded.extend(parts)
                        continue
    ...
    for i, cand in enumerate(candidates):
        current += cand
        if (len(current) >= target_per_block and len(blocks) < n_blocks - 1) or (len(blocks) + 1 == n_blocks and i == len(candidates) - 1):
    ...
    # normaliser en dicts
    normalized = [{"index": idx, "html": normalize_html_minify_fragment(b)} for idx, b in enumerate(blocks)]
    return normalized
...
def process_file(src_path: Path, out_dir: Path, target_per_block: int = 40000, max_blocks: int = 10):
    name = src_path.stem
    try:
        ...
        blocks = split_blocs_text(minified, target_per_block=target_per_block, max_blocks=max_blocks) if 'split_blocs_text' in globals() else split_blocs(src_path, target_per_block=target_per_block, max_blocks=max_blocks)
...
def run(input_dir: Optional[Path] = None, out_dir: Optional[Path] = None, target_per_block: int = 70000, max_blocks: int = 5):
    """
    ...
    if catalogue:
        print(f"Catalogue chargé ({len(catalogue)} entrées). Les fichiers marqués 'inutile' seront sautés.")
    else:

...
def compare_files(expected_file: Path, llm_file: Path) -> Dict[str, Any]:
    ...
    if not llm_file.exists():
        return {"status": "missing", "message": f"❌ Fichier LLM manquant: {llm_file.name}"}
...
def generate_comparison_report():
    ...
    if files_with_errors > 0 or false_positives > 0 or false_negatives > 0:
        ...
        if false_positives > 0:
            ...
            print(
                "   - Clarifier la différence entre 'texte descriptif' et 'modification réglementaire'"
            )
            print()
        ...
        if "source_article" in error_types:
            ...
            print(
                "   - Ajouter dans le prompt: 'Si l'opération provient d'une annexe (balise <footer data-spec=\"appendix\">), mettre source_article = \"APPENDIX\"'"
            )
            print()
        ...
        if "sub_target" in error_types:
            ...
            print(
                '   - Clarifier dans le prompt: \'sub_target peut être "ALL" (remplacer tout l\'article), "END" (ajouter à la fin), ou null (précision via markers)\''
            )
            print()

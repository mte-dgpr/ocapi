...
from ocapi.types import ArreteId, ImageMap, RawOperation, ArreteFile, Operation
...
if __name__ == "__main__": 
            ...
            for chunk in doclist[i]:
                for j, raw_op in enumerate(raw_operations):
                    try:
                        ...
                        print(f"  [!] Skipping operation {j+1}/{len(raw_operations)}: {raw_op.operation_type} {raw_op.source_article}->{raw_op.target_article} - {str(e)[:80]}")
            
            success_count = len(all_ops) - ops_count_before
            print(f"  Converted {success_count}/{len(raw_operations)} operations from {len(doclist[i])} chunks")
            if failed_ops:
    ...
    if all_failed_ops:
        ...
        for fail in all_failed_ops:
            print(f"  - {fail['file']}: op {fail['operation_index']} ({fail['operation_type']} {fail['source_article']}->{fail['target_article']})")

...
def make_contenu_permis(history: ArticleHistory, arrete_files: list[ArreteFile],
    ...
    for section in sections:
        ...
        section.replace_with(make_consolidated_section(section, article_id, operations, history, ap_initial_id))
    ...
    for i, version in enumerate(versions):
        ...
        if i == 0:
            ...
            if op_id:
                op = next((o for o in operations if o.id == op_id), None)
                if op:
                    op_type_text = "modifié" if op.operation_type == "REPLACE" else "abrogé" if op.operation_type == "REMOVE" else "créé"
                    source_article = f"Article {op.source_id.article_id}" if op.source_id.article_id else "l'arrêté"

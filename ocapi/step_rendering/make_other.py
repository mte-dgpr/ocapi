...
def detect_additional_prescriptions(arrete_files: list[ArreteFile]) -> str:
    # TODO : refaire des appels LLM pour détecter les prescriptions additionnelles
    # non modificatives ? à voir.
    _ = arrete_files
    return ""
    ...
    for arrete_file in arrete_files:
        if arrete_file.ordered_index > 0:
            if arrete_file.status and has_no_ops(arrete_file, operations):
                other_str += (
                    f"Contenu de l'arrêté complémentaire {arrete_file.id} : "
                    f"{extract_main(arrete_file.soup)}"
                )
    return other_str

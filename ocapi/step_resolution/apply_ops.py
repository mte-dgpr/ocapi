...
from ocapi.step_detection.subtarget_detection import (
    is_simple_subtarget,
    parse_subtarget,
    replace_subtarget,
)
...
def apply_replace(operation: Operation, soup_input: BeautifulSoup) -> Content:
    if is_simple_subtarget(operation.sub_target):
        ...
        prompt = query_llm_for_subtarget(
            OperationType.REPLACE, str(soup_input), operation.sub_target.description
        )
...
def apply_remove(operation: Operation, soup_input: BeautifulSoup) -> Content:
    ...
    if is_simple_subtarget(parsed):
        ...
        prompt = query_llm_for_subtarget(
            OperationType.REMOVE, str(soup_input), operation.sub_target.description
        )
        raw = call_llm_api(prompt)
        for line in raw.splitlines():
            if "<NEWCONTENT>" in line:
...
def apply_add(operation: Operation, soup_input: BeautifulSoup) -> Content:
    ...
    if is_simple_subtarget(parsed):
        ...
        prompt = query_llm_for_subtarget(
            OperationType.ADD, str(soup_input), operation.sub_target.description
        )
        raw = call_llm_api(prompt)
        for line in raw.splitlines():
            if "<NEWCONTENT>" in line:
